...
import com.iv.form.binding.BinderConfiguration;
...
/**
 * @author liangk 2018年5月7日 form-service-api-1.0.0-SNAPSHOT
 */
@Service
// @Transactional
public class FormService {
	...
	public FormInfoEntity saveOrUpdateFormInfo(FormMsgDto formMsgDto, int userId) throws BusException {
		...
		// 校验
		if (StringUtils.isEmpty(formMsgDto.getDemandTypeCode()) || StringUtils.isEmpty(formMsgDto.getFormApplyTime())
				|| StringUtils.isEmpty(formMsgDto.getFormExpectEndTime())
				|| StringUtils.isEmpty(formMsgDto.getFormOwnerId())
				|| StringUtils.isEmpty(formMsgDto.getFormOwnerPhone()) || StringUtils.isEmpty(formMsgDto.getHandlerId())
				|| StringUtils.isEmpty(formMsgDto.getUnitCode()) || StringUtils.isEmpty(formMsgDto.getPriority())) {
		...
		// 发送待处理消息通知
		msgSender.msgSend(Arrays.asList(formInfoEntity.getHandlerId()), formInfoEntity.getId(),
				formInfoEntity.getDemandContent(),
				Constant.FORM_STATE_CONTENT_INIT, MsgType.FORM_PENDING);
	 ...
	 * @throws BusException
	 */
	public FormInfoPage selectMyFormListPage(HttpServletRequest request, int userId, FormConditionDto formConditionDto)
			throws BusException {
		// 校验参数
		if (StringUtils.isEmpty(userId)) {
			throw new BusException(ErrorMsg.FORM_HANDLER_ID_FAILED);
		...
		// 添加工单记录
		List<LinkedHashMap> data = groupServiceClient.groupsUsersInfo() == null ? null
				: (List<LinkedHashMap>) groupServiceClient.groupsUsersInfo().getData();
	 ...
	 * @throws BusException
	 */
	public FormInfoPage selectDelFormListPage(HttpServletRequest request, int userId, FormConditionDto formConditionDto)
			throws BusException {
		// 校验参数
		if (StringUtils.isEmpty(userId)) {
			throw new BusException(ErrorMsg.FORM_HANDLER_ID_FAILED);
	 ...
	 * @throws BusException
	 */
	public FormInfoPage selectMarkFormListPage(HttpServletRequest request, int userId,
			FormConditionDto formConditionDto) throws BusException {
		// 校验参数
		if (StringUtils.isEmpty(userId)) {
			throw new BusException(ErrorMsg.FORM_HANDLER_ID_FAILED);
		}
	...
	public FormInfoPage selectFormListPage(HttpServletRequest request, FormConditionDto formConditionDto,
			Integer userId) {
		// 设置默认当前页和间距
		formConditionDto.setCurPage(formConditionDto.getCurPage() == 0 ? 1 : formConditionDto.getCurPage());
		formConditionDto.setItems(formConditionDto.getItems() == 0 ? 10 : formConditionDto.getItems());
	...
	public void executeTakeOver(String formId, int userId) throws BusException {
		...
		//
		// String taskId = formActService.formActService(variables, userId,
		// user.getRealName());
	 ...
	 */
	// @Transactional
	public void executeDealWithEnd(FormOperateLogsDto formOperateLogsDto, int userId, String curTenantId)
			throws BusException {
		// 校验参数
		...
		// 查询审核人员
		Set<LocalAuthDto> localAuthDtos = subTenantPermissionServiceClient.approveFormPerson("90120", curTenantId,
				formInfoEntity.getGroupId().shortValue());
		...
		localAuthDtos.forEach(n -> approverIds.add(n.getId()));
		msgSender.msgSend(approverIds, formInfoEntity.getId(), formInfoEntity.getDemandContent(),
				Constant.FORM_STATE_CONTENT_AUDIT, MsgType.FORM_REVIEW);
	 ...
	 *            转派人员Id
	 */
	public void executeUpgrade(int userId, String formId, Integer handlerGroupId, Integer handlerId, String reason)
			throws BusException {
		// 校验参数
		...
		msgSender.msgSend(Arrays.asList(formInfoEntity.getHandlerId()), formInfoEntity.getId(),
				formInfoEntity.getDemandContent(),
				Constant.FORM_STATE_CONTENT_UPGRADE, MsgType.FORM_PENDING);
		String code = formInfoEntity.getId() + "\\&" + JWTUtil.getReqValue("curTenantId");
		wechatService.sendWechat(formInfoEntity, code);
	...
	public void executeAuditPass(int userId, FormAuditDto formAuditDto) throws BusException {
		...
		emailService.sendEmail(formInfoEntity);
		// 发送待评价通知
		msgSender.msgSend(Arrays.asList(formInfoEntity.getApplicantId()), formInfoEntity.getId(),
				formInfoEntity.getDemandContent(),
				Constant.FORM_STATE_CONTENT_EVALUATE, MsgType.FORM_EVALUATE);
	 ...
	 * @param formConditionDto
	 */
	public void FormExport(HttpServletRequest request, HttpServletResponse response,
			FormConditionDto formConditionDto) {
