...
import com.iv.message.dao.IApproveFlowMsgDao;
import com.iv.message.entity.ApproveFlowMsgEntity;

@Repository
public class ApproveFlowMsgDaoImpl implements IApproveFlowMsgDao {

	@Override
	public void save(ApproveFlowMsgEntity entity) throws RuntimeException {
	...
	public ObjectPageDto selectByUserId(int userId, int firstResult, int maxResults) throws RuntimeException {
		return (ObjectPageDto) HibernateTemplateWithTenant.execute(new HibernateCallBack() {
			...
			public Object doInHibernate(Session ses) throws HibernateException {
				...
				String hql = "from ApproveFlowMsgEntity a where a.userId=? and a.confirmed=? order by a.msgDate desc";
				int num = -1; //查询所有数据
				if(num == maxResults) {
	...
	public void updateConfirmed(List<String> ids, boolean confirmed) throws RuntimeException {
		HibernateTemplateWithTenant.execute(new HibernateCallBack() {
			...
			@Override
			public Object doInHibernate(Session ses) throws HibernateException {
				int num = ses.createQuery("update ApproveFlowMsgEntity a set a.confirmed=? where a.id in (:idList)")
	...
	public void updateConfirmedAllByUserId(int userId, boolean confirmed) throws RuntimeException {
		HibernateTemplateWithTenant.execute(new HibernateCallBack() {
			...
			@Override
			public Object doInHibernate(Session ses) throws HibernateException {
				int num = ses.createQuery("update ApproveFlowMsgEntity a set a.confirmed=? where a.userId=?")
