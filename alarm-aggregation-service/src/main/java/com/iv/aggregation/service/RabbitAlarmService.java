...
public class RabbitAlarmService {
	...
	public void processZabbixQueue(AlarmMessageInput ami) {
		...
		LOGGER.warn("#告警！ [来源：zabbix ]--" + "[token]:" + ami.getTenantId() + " -- " + new Date().toLocaleString());
		if (!isAccept(ami.getTenantId())) {
			...
			try {
				alarmLifeEntity = zabbixAlarmHandler.alarmProcessing(ami);
			} catch (Exception e) {
	...
	public void processNagiosQueue(NagiosMessageInput nmi) {
		LOGGER.warn("#告警！ [来源：nagios ]--" + "[租户ID]:" + nmi.getTenantId() + " -- " + new Date().toLocaleString());
		if (!isAccept(nmi.getTenantId())) {
			...
			AlarmLifeEntity alarmLifeEntity = null;
			try {
				alarmLifeEntity = nagiosAlarmHandler.alarmProcessing(nmi);
	...
	private boolean isAccept(String token) {
